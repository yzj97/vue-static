    ...
    <ody-list-table-area>
      ...
      <div slot="table">
        <ody-table
          ...
          name="table_data752"
        >
          <template slot="type" slot-scope="scope">{{ scope.row.type | keyVal(getTypeMap) }}</template>
          <template
            slot="subType"
            slot-scope="scope"
          >{{ (scope.row.type.toString() + '_'+ scope.row.subType.toString()) | keyVal(getSubTypesMap) }}</template>
...
export default {
  ...
  async mounted() {
    await this.query()
    await this.$ouserCenter.$api.userService
  ...
  methods: {
    ...
    query() {
      ...
      params.entityType = 2
      this.$ouserCenter.$api.crmService.listRulePage(params).then(res => {
    ...
    addNewRule() {
      this.$router.push({
        path: '/ouser-center/growthValueOperation',
        query: {
          add: 1,
          edit: 1
        }
