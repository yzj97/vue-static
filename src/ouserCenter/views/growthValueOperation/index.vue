...
export default {
  ...
  async mounted() {
    await this.$ouserCenter.$api.growthValueList.queryChannelMap().then((res) => {
      const data = res.data
      for (const key in data) {
        this.channelList.push({ id: key, label: data[key].channelName })
      }
    })
    await this.$ouserCenter.$api.crmService.listMemberType({}).then(res => {
      this.memberTypes = res.data
    })
    await this.$ouserCenter.$api.userService.configListMulti({
  ...
  methods: {
    async init() {
      if (this.$route.query.add.toString() === '1') {
        ...
        this.$ouserCenter.$api.crmService.getPointRuleById({ id: this.$route.query.id }).then(res => {
    ...
    updateStatus(rule, status) {
      this.$confirm('确认启用/停用该条数据?', this.$t('提示'), {
        ...
        const growthValueList = this.$ouserCenter.$api.growthValueList
