      ...
      <ody-list-search-area>
        <div slot="content">
          <el-form ref="form" label-width="100px" class="form">
            <ody-search-item :label="$t('member_cell_phone_number')" class="register" prop="mobile">
              <el-input v-model="searchForm.mobile" name="searchForm_mobile" type="text" />
            </ody-search-item>
        ...
        <div slot="btn">
          ...
          <ody-button :code="type === 2 ? 'OuserCenterBalanceFlowListQuery' : 'OuserCenterPointsFlowQuery'" name="handleSearchSubmit" size="small" type="primary" @click="handleSearchSubmit">{{ $t('query') }}</ody-button>
        </div>
      </ody-list-search-area>
      <ody-list-table-area>
        <div slot="table">
        ...
        list: [
          {
            ...
            code: this.type === 2 ? 'OuserCenterBalanceFlowListSee' : 'OuserCenterPointsFlowSee'
          }
        ]
      }
    }
...
async function loadList(vue) {
  ...
  let op
  if (vue.type === 2) {
    op = vue.$ouserCenter.$api.crmService.listGrowthAccountPage(vue.searchForm)
  } else {
    op = vue.$ouserCenter.$api.crmService.listPointAccountPage(vue.searchForm)
  }
  return op.then(res => {
    if (res.code === '0') {
