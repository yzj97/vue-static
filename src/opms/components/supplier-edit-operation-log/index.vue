<template>
  <div class="supplier-edit-operation-log">
    <ody-list-table-area>
      <div slot="table">
        <ody-table :data="listTable.list" :can-filter="false" :columns="listTable.columns" name="listTable_list980" />
      </div>
      <div slot="page">
        <div class="block">
          <ody-pagination
  ...
  methods: {
    async query() {
      if (this.supplierId) {
        ...
        this.$opms.$api.supplier.listOperActionPage(searchParams).then(res => {
