  ...
  <div class="supplier-edit-address-info">
    ...
    >
      ...
      <el-form-item :label="$t('详细地址')" prop="detailAddress">
        <el-input v-model="deliveryAddressInfo.detailAddress" name="deliveryAddressInfo_detailAddress" maxlength="300" />
      </el-form-item>
    </el-form>
  </div>
</template>
...
export default {
  ...
  async mounted() {
    if (this.isEdit || this.isView) {
      ...
      } = await this.$opms.$api.supplier.queryMerchantOrgAddressByOrgId(
        this.supplierId + ''
      )
      const {
        provinceCode,
