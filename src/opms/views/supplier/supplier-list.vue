    ...
    <ody-list-search-area>
      <div slot="content">
        <el-form ref="search" :model="search">
          ...
          <ody-search-item :space="2" :label="$t('创建时间')" prop="obj.dateRangePicker">
          ...
          list: [
            ...
            },
            {
              ...
              method: async(index, row) => {
                try {
                  await this.$confirm('您确认要停用', this.$t('提示'), {
                    confirmButtonText: this.$t('确定'),
                    cancelButtonText: this.$t('取消'),
                    type: 'warning'
                  })
                  await this.$opms.$api.supplier.updateDistributionStatus({
                    orgId: row.orgId,
                    status: 0
                  })
                  this.$message({
            ...
            },
            {
              ...
              method: async(index, row) => {
                try {
                  await this.$confirm('您确认要启用', this.$t('提示'), {
                    confirmButtonText: this.$t('确定'),
                    cancelButtonText: this.$t('取消'),
                    type: 'warning'
                  })
  ...
  methods: {
    ...
    exportSupplier() {
      ...
      if (this.search.obj.dateRangePicker) {
        if (this.search.obj.dateRangePicker[0]) {
          params.createTimeStart = this.search.obj.dateRangePicker[0]
        } else {
          params.createTimeStart = null
        }
        if (this.search.obj.dateRangePicker[1]) {
          params.createTimeEnd = this.search.obj.dateRangePicker[1]
        } else {
    ...
    getFilters() {
      ...
      var searchObj = {
        ...
        orgName: vue.search.obj.orgName,
        orgCode: vue.search.obj.orgCode,
        supplierClassificationNode: vue.search.obj.supplierClassificationNode,
        bussinessType: vue.search.obj.bussinessType,
      ...
      if (filters.dateRangePicker) {
        ...
        if (filters.dateRangePicker[0]) {
          filters.obj.createTimeStart = filters.dateRangePicker[0]
        }
        if ((filters.obj.createTimeEnd = filters.dateRangePicker[1])) {
          filters.obj.createTimeEnd = filters.dateRangePicker[1]
        }
    ...
    handleSearchReset() {
      ...
      this.search.obj.dateRangePicker = []
