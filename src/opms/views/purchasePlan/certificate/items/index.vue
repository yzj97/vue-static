<template>
  <section class="context">
    ...
    <div v-if="data1 != null && data1.length > 0">
      <div class="title-div">
    ...
    <ody-button
      ...
      type="primary"
      size="small"
      code="OpmsPurchasePlanCertificate04"
      style="margin-left: 10px;margin-bottom: 10px;"
      @click="auditorDialog = true"
    >{{ $t('common_batch_audit') }}</ody-button>
    <ody-table
      :data="data2"
      :columns="columns"
      :operates="operates"
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <ody-button
          name="OpmsPurchasePlanCertificate01_doAuditor"
          size="small"
          code="OpmsPurchasePlanCertificate01"
          @click="doAuditor(0)"
        >{{ $t('notpass') }}</ody-button>
        <ody-button
          name="OpmsPurchasePlanCertificate02_doAuditor"
          size="small"
          type="primary"
          code="OpmsPurchasePlanCertificate02"
          @click="doAuditor(1)"
        >{{ $t('pass') }}</ody-button>
      </span>
    </ody-dialog>
    ...
    <ody-dialog :visible.sync="dialogFormVisible" :title="$t('common_edit')" width="650px">
      <el-form :model="form">
        <el-form-item
          ...
          :label="$t('supplier_attach_type_qualification')"
        >
          <ody-upload-image
            :file-list.sync="form.certificateUrl"
            :pic-type="['png', 'jpg', 'gif']"
            @success="uploadImgChange"
          />
          <span class="span-prompt">{{ $t('upload_images_specification') }}</span>
        </el-form-item>
        <el-form-item
          :label-width="formLabelWidth"
          :label="$t('supplier_attach_qualification_code')"
        >
          <el-input
            v-model="form.certificateCode"
            name="form_certificateCode"
            maxlength="100"
            autocomplete="off"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button
          name="dialogFormHandle"
          @click="dialogFormHandle(false)"
        >{{ $t('common_cancel') }}</el-button>
        <ody-button
          name="OpmsPurchasePlanCertificate03_dialogFormHandle"
          type="primary"
          code="OpmsPurchasePlanCertificate03"
          @click="dialogFormHandle(true)"
...
export default {
      ...
      columns: [
        ...
        },
        {
          ...
          formatter: (row, column, cellValue) => {
            return `<a style="white-space: nowrap;color: dodgerblue;" href="${
              row.certificateUrl
            }">${
              row.certificateUrlName != null ? row.certificateUrlName : ''
            }</a>`
  ...
  async mounted() {
    try {
      purchasePlanId = this.$route.query.purchasePlanId
  ...
  methods: {
    ...
    doAuditor(auditorStatus) {
      ...
      for (let i = 0; i < this.checked.length; i++) {
        ...
        if (auditorStatus === 1) {
          if (obj.needCert === 1 && !obj.certificateUrl) {
            // 证书必传
            this.message =
              this.$t('plMpResult_mpCode') +
              obj.mpCode +
              this.$t('cert_is_not_null')
          ...
          if (obj.needCert === 1 && !obj.certificateCode) {
            ...
            this.message =
              this.$t('plMpResult_mpCode') +
              obj.mpCode +
              this.$t('cert_code_is_not_null')
            this.dialogVisible = true
            return
          }
          if (
            obj.needExpiryDate === 1 &&
            (!obj.certificateStartDate || !obj.certificateEndDate)
          ) {
            // 证书有效期必填
            this.message =
              this.$t('plMpResult_mpCode') +
              obj.mpCode +
              this.$t('cert_validity_date_is_not_null')
            this.dialogVisible = true
            return
          }
        }
      ...
      if (list.length > 0) {
        ...
        purchasePlan
          .updatePlanItemsCertificateAuditorStatus({ list: list })
          .then(data => {
    ...
    handleEdit(index, row) {
      ...
      if (row.certificateStartDate == null || row.certificateEndDate == null) {
        ...
        this.form.certificateDate = [
          row.certificateStartDate,
          row.certificateEndDate
        ]
      }
      ...
      if (ok && this.dialogFormVisible) {
        ...
        if (this.form.certificateUrl.length > 0) {
          ...
          if (
            ...
            data.certificateUrlName === ''
          ) {
            data.certificateUrlName = data.certificateUrl.substring(
              data.certificateUrl.lastIndexOf('/') + 1
            )
            console.log(data.certificateUrlName)
            if (data.certificateUrlName.indexOf('@')) {
              data.certificateUrlName = data.certificateUrlName.substring(
                0,
                data.certificateUrlName.indexOf('@')
              )
            }
            if (data.certificateUrlName.length > 50) {
              data.certificateUrlName = data.certificateUrlName.substring(
                data.certificateUrlName.length - 50
              )
            }
          }
          if (
        ...
        if (data.certificateUrl && (data.certificateUrlName == null ||
            data.certificateUrlName === '')) {
          data.certificateUrlName = this.form.certificateUrlName
        }

        console.log('req：' + JSON.stringify(data))
        const purchasePlan = this.$opms.$api.purchasePlan
        purchasePlan.updatePlanItemsCertificateInfo(data).then(res => {
...
async function loadListPage(vue) {
  ...
  const res = await purchasePlan.purchasePlanItemsCertificateListPage(param)
  if (res) {
    vue.data1 = res.data
...
async function loadList(vue) {
  ...
  const purchasePlan = vue.$opms.$api.purchasePlan
  const res = await purchasePlan.purchasePlanItemsCertificateList(param)
  if (res) {
    vue.data2 = res.data
  }
}
