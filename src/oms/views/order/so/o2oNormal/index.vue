...
export default {
  ...
  methods: {
    async  init() {
      ...
      this.stepInfo = [
        { title: this.$t('manual_order_submit'), desc: self.soDetail.so.orderCreateTime, active: false }, // 提交订单
        { title: this.$t('payment_completion'), desc: self.soDetail.so.orderPaymentConfirmDate, active: false }, // 支付完成
        { title: this.$t('商家接单'), desc: null, active: false },
