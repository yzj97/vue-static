  ...
  methods: {
    async  init() {
      ...
      await this.$oms.$api.so.soDetailGet({ orderCode: this.$route.params.orderCode }).then(res => {
        ...
        } else if (so.orderPaymentStatus === 2 && so.orderStatus < 1030) {
          activeStep = 1
          buttonKeys = ['cancel', 'edit']
      ...
      this.stepInfo = [
        ...
        { title: this.$t('支付定金'), desc: null, active: false },
        { title: this.$t('支付尾款'), desc: self.soDetail.so.orderPaymentConfirmDate, active: false },
