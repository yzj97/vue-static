  ...
  <div>
    <ody-list-search-area>
      <div slot="content">
        <el-form ref="search" :model="search" label-width="100px" class="form">
          <ody-search-item :label="$t('店铺') + ':'" prop="storeId">
            <ody-store-select ref="store" v-model="search.storeId" name="search_storeId" value-key="storeId" />
          </ody-search-item>
      ...
      <div slot="btn">
        ...
        <ody-button
          name="PriceAuditStoreStoreMpAudit_btn"
          code="PriceAuditStoreStoreMpAudit_btn"
          size="small"
          type="primary"
    ...
    <ody-list-table-area>
      <div slot="btn-bottom">
        <ody-button
          ...
          name="PriceAuditStoreAduit_btn_showAuditDialog"
          type="primary"
          code="PriceAuditStoreAduit_btn"
          size="small"
      ...
      <div slot="table">
        <ody-table
          ...
          request-url="/back-product-web/back/price/mpPriceAudit/listStoreMpAudit.do"
        >
          <template slot="viewPrice" slot-scope="scope">
            <ody-button
              name="PriceAuditStoreAduitDetail_btn_showAuditDialog"
              code="PriceAuditStoreAduitDetail_btn"
              type="text"
    ...
    >
      <el-card v-if="audit.queryDetail" shadow="never">
        <ody-table
          ...
          :can-filter="false"
          name="audit_table_detailData662"
          height="200px"
        />
      </el-card>
      <el-form
      ...
      <span slot="footer" class="dialog-footer">
        <el-row v-if="!checkedItem || checkedItem.status === 0" type="flex" justify="end">
          ...
          <ody-button
            ...
            name="PriceAuditStoreAduit_btn_handleOk"
            code="PriceAuditStoreAduit_btn"
            size="small"
            type="primary"
            @click="handleOk"
        ...
        <el-row v-if="checkedItem && checkedItem.status !== 0" type="flex" justify="end">
          <el-button name="handleClose0" size="small" type="primary" @click="handleClose">{{ $t('确定') }}</el-button>
        </el-row>
      </span>
    </ody-dialog>
  </div>
...
export default {
        ...
        columns: [
          {
            ...
            render: (h, params) => {
              return (
                <div>
                  {vue.storeMap[params.row.storeId]
                    ? vue.storeMap[params.row.storeId].storeName
                    : ''}
                </div>
              )
            }
  ...
  methods: {
    ...
    async initStore() {
      const {
        data
      } = await this.$product.$api.merchantApi.queryPlatformAuthStorePage({
        currentPage: 1,
        itemsPerPage: 1000
      })
      if (data && data.listObj) {
        data.listObj.forEach(element => {
          this.storeMap[element.storeId] = element
        })
      }
    },
    handleReset() {
    ...
    async queryDetail() {
      ...
      let { data } = await this.$product.$api.priceApi.listMerchantProductPriceAuditDetailByPriceAuditId(params)
      data = data || []
      if (data.length > 0 && data[0].modifyType === 5 && data[0].type === 2) {
        if (data[0].beforeAmount == null) {
          data[0].beforeAmount = 0
        }
        data.push({
          modifyType: 51,
          afterValue: `${data[0].amount}`,
          beforeValue: `${data[0].beforeAmount}`
        })
      }
    ...
    async queryCount() {
      ...
      params.createTimeEnd = createTimeEnd
        ...
        : createTimeEnd
      params.status = null
      const { data } = await this.$product.$api.priceApi.countStoreMpAuditMap(
        params
      )
      ...
      this.$refs.detailForm.validate(validate => {
        if (validate) {
          ...
          vue.$product.$api.priceApi
            .batchUpdateStoreStatusWithTx(params)
            .then(res => {
              vue.handleClose()
              vue.query()
            })
