  ...
  <div>
    ...
    >
      <section>
        <el-form ref="batchCansaleForm" :model="batchCansaleForm" :rules="rules" class="form">
          <div class="content-up">
            ...
            <div v-if="type===0">
              <el-form-item prop="chooseFlag">
                <el-radio-group v-model="batchCansaleForm.chooseFlag" name="batchCansaleForm_chooseFlag1" @change="changeFlag">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <ody-button
          v-if="type===1"
          name="StoreProductListShelf_submitForm"
          size="small"
          type="primary"
        ...
        <ody-button
          v-if="type===0"
          name="StoreProductListDown_submitForm"
          size="small"
          type="primary"
      ...
      this.$refs['batchCansaleForm'].validate(valid => {
        if (valid) {
          ...
          if (this.batchCansaleForm.chooseFlag === 2) {
            ...
            if (submitParams.canSale === 1) {
              submitParams.storeIdList = this.storeidlist
              submitParams.merchantProductIdList = this.merchantproductidlist
              submitParams.merchantIdList = this.merchantidlist
              submitParams.mpNameList = this.mpnamelist
            }
          } else {
            submitParams.mpCodeList = codelList
          }
          this.$product.$api.mpApi
            .redevStoreMpCanSale(submitParams)
            .then(result => {
