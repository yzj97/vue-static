    ...
    <ody-dialog
      ...
      <el-form ref="ruleForm" :model="params" :rules="rules" label-position="right" label-width="180px">
        ...
        <el-form-item v-if="categoryType === 1" :label="$t('允许自定义属性')" prop="enableAddAtt">
          <el-radio v-model="params.enableAddAtt" :label="1" name="params_enableAddAtt">{{ $t('是') }}</el-radio>
          <el-radio v-model="params.enableAddAtt" :label="0" name="params_enableAddAtt9">{{ $t('否') }}</el-radio>
        </el-form-item>
        <el-form-item v-if="categoryType !== 1" :label="$t('状态')" prop="enableAddAtt">
          <el-radio v-model="params.isVisible" :label="1" name="params_isVisible">{{ $t('显示') }}</el-radio>
          <el-radio v-model="params.isVisible" :label="0" name="params_isVisible8">{{ $t('隐藏') }}</el-radio>
        </el-form-item>
        <el-form-item v-if="categoryType !== 1" :label="$t('图片')" prop="enableAddAtt">
          <ody-upload-image v-model="params.pictureUrl" :pic-type="picType" :pic-size="4096" :multiple="false" name="params_pictureUrl" @error="error"/>
        </el-form-item>
        <el-form-item v-if="categoryType !== 1" label="" prop="enableAddAtt">
...
export default {
  ...
  methods: {
    ...
    validatePass(rule, value, callback) {
      if (this.params.lowPrice === null) {
        this.params.lowPrice = ''
      }
      if (this.params.highPrice === null) {
