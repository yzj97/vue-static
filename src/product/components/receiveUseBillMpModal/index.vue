...
export default {
  ...
  data() {
    return {
      ...
      queryParam: { merchantId: this.merchantId, warehouseId: this.warehouseId }
    }
  },
  mounted() {
  ...
  methods: {
    ...
    query() {
      ...
      this.stockApi.listReceiveUseBillMpByPage(param).then(
        res => {
    ...
    reset() {
      this.queryParam = { merchantId: this.merchantId, warehouseId: this.warehouseId }
    },
    sizeChange(val) {
