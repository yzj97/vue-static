    ...
    <el-tabs v-model="activeName" name="activeName">
      ...
      <el-tab-pane :label="$t('店铺')" name="store">
        <div class="activity-merchant">
          <div>
            <ody-store-choose
              v-if="merchantTable.data.length>0"
              :key="merchantTable.data[0].merchantId"
              v-model="storeSelected"
              :selected.sync="storeSelectedItem"
              :multiple="true"
              :params="{ merchantIds: [merchantTable.data[0].merchantId]}"
              :search-field="['merchantId', 'storeCode', 'storeName']"
              name="storeSelected"
              re-url="/ouser-web/api/store/queryStoreOrgPageByAuth.do"
              value-key="storeId"
              class="add-button"
...
export default {
  ...
  methods: {
    ...
    async handleMerchantSubmit(merchantType) {
      if (merchantType === 1) {
        ...
        if (this.allAuthMerchant.length > 0) {
          ...
          this.$emit(
            'update:getmerchantId',
            this.merchantTable.data[0].merchantId
          )
        } else {
    ...
    async deleteMerchantListUrl(row, merchantType) {
      ...
      const { code } = await this.promotionAPI.delMerchantList({
    ...
    async addMerchantList(merchantAddList, merchantType) {
      ...
      await this.promotionAPI.addCouponMerchantList(params)
