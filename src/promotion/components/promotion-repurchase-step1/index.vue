<template>
  <div>
    <el-form
      ref="repurchasePromotion"
      :model="repurchasePromotion"
      :rules="repurchasePromotionRules"
      label-width="240px"
      class="form"
    >
      ...
      <div class="single-promotion-content">
        <el-form-item :label="$t('促销活动名称')" prop="promTitle" name="promTitle">
          <el-input
            v-model="repurchasePromotion.promTitle"
            :placeholder="$t('请输入促销活动名称')"
            name="repurchasePromotion_promTitle"
            maxlength="64"
            show-word-limit
          />
        </el-form-item>
        <el-form-item :label="$t('活动时间')" prop="createTime" name="createTime">
          <el-date-picker
            v-model="repurchasePromotion.createTime"
            name="repurchasePromotion_createTime"
      ...
      <div class="single-promotion-content">
        <el-form-item
          ...
          required
        >
          <promotion-repurchase-type-radio v-model="repurchasePromotion.conditionType" name="repurchasePromotion_conditionType" />
        </el-form-item>
        <el-form-item :label="$t('活动描述(中文)')" prop="ruleDesc" name="ruleDesc">
          <promotion-activity-description-input-chinese
            :rule-desc.sync="repurchasePromotion.ruleDesc"
        ...
        <el-form-item :label="$t('活动描述(英文)')" prop="ruleDesc" name="ruleDesc">
          <promotion-activity-description-input-english
            :rule-desc-lan.sync="repurchasePromotion.ruleDescLan2"
      ...
      <div class="single-promotion-content">
        ...
        >
          <ody-radio2
            v-model="repurchasePromotion.canUseCoupon"
            name="repurchasePromotion_canUseCoupon"
        ...
        <el-form-item v-show="platformId != 2" :label="$t('是否包邮')" prop="freeShipping" name="freeShipping" required>
          <ody-radio2
            v-model="repurchasePromotion.freeShipping"
            name="repurchasePromotion_freeShipping"
            list-key="promotionFreeShippingList"
          />
        </el-form-item>
        <!-- <el-form-item :label="$t('超限规则')" prop="overlimitType" required>
          <promotion-over-limit-radio
            v-model="repurchasePromotion.overlimitType"
            name="repurchasePromotion_overlimitType"
          />
        </el-form-item> -->
      </div>
      ...
      <div class="single-promotion-content">
        <el-form-item :label="$t('活动渠道')" prop="channelCodes" name="channelCodes" required>
          <promotion-channel-code-checkbox
            v-model="repurchasePromotion.channelCodes"
            :is-show-all="true"
            :checked-all="promotionId === ''"
            :prom-type="promType"
            name="repurchasePromotion_channelCodes"
          >
        ...
        >
          <promotion-applicable-platform-checkbox
            v-model="repurchasePromotion.applicablePlatformList"
            :is-show-all="true"
            :checked-all="promotionId === ''"
            name="repurchasePromotion_applicablePlatformList"
          />
        </el-form-item>
        <el-form-item v-show="platformId != 2" :label="$t('新老用户')" prop="userScopeList" name="userScopeList">
          <promotion-user-scope-checkbox
            v-model="repurchasePromotion.userScopeList"
            :is-show-all="true"
            :checked-all="promotionId === ''"
            name="repurchasePromotion_userScopeList"
          />
        </el-form-item>
        <el-form-item v-show="platformId != 2" :label="$t('会员类型')" prop="memberTypes" name="memberTypes">
          <promotion-member-types-checkbox
            v-model="repurchasePromotion.memberTypes"
            :selected.sync="memberTypesSelected"
            :checked-all="promotionId === ''"
            name="repurchasePromotion_memberTypes"
          />
        </el-form-item>
        <el-form-item v-show="platformId != 2" :label="$t('会员等级')" prop="memberLevels" name="memberLevels">
          <promotion-member-levels-checkbox
            v-model="repurchasePromotion.memberLevels"
            :checked-all="promotionId === ''"
            name="repurchasePromotion_memberLevels"
            @change="handleMemberLevelsChange"
          />
        </el-form-item>
        <el-form-item :label="$t('备注')" prop="description" name="description">
          <ody-tinymce :show-upload-img="false" :height="300" v-model="repurchasePromotion.description" :limit="1000" name="repurchasePromotion_description" />
        </el-form-item>
      </div>
    </el-form>
...
import PromotionRepurchaseTypeRadio from '@/components/promotion-repurchase-type-radio'
...
export default {
  ...
  watch: {
    'repurchasePromotion.memberTypes': {
      handler: function(val) {
        this.$nextTick(() => {
          if (this.memberTypesSelected.length > 0) {
            this.repurchasePromotion.memberLevels = this.memberTypesSelected.reduce((rtv, item) => {
          ...
          if (val && val.length <= 0) {
            this.repurchasePromotion.memberLevels = []
          }
        })
      },
      deep: true
    },
    'repurchasePromotion.conditionType': {
      handler(val) {
        var params = {
          conditionType: val,
          favourableList: [],
  ...
  methods: {
    handleMemberLevelsChange(type, isChecked) {
      const beforeIndex = this.repurchasePromotion.memberTypes.indexOf(type)
      const beforeTypeIndex = this.memberTypesSelected.findIndex((item) => {
        return item.value === type
      })
      if (isChecked) {
        if (beforeIndex === -1) {
          this.repurchasePromotion.memberTypes.push(type)
          this.memberTypesSelected = []
        }
      } else {
        if (beforeIndex >= 0) {
          this.repurchasePromotion.memberTypes.splice(beforeIndex, 1)
          this.memberTypesSelected.splice(beforeTypeIndex, 1)
        }
      }
    },
    async queryPromotionActivityDetail() {
      if (this.repurchasePromotion.id === '') {
        return
      }

      const { data = {}, data: { fromDate = '', endDate = '' }, code } = await this.promotionAPI.getPromotionDetailUrl(this.repurchasePromotion)

      if (code === '0') {
        this.repurchasePromotion = data
        this.$set(this.repurchasePromotion, 'createTime', [fromDate, endDate])
        this.$emit('repurchasePromotion', this.repurchasePromotion)
      }
    }
  }
}
