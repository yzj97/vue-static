<template>
  <div class="pg-member-restrictions">
    <ody-box :title="$route.path.indexOf('edit') > 0 ? $t('编辑限购规则') : $t('创建限购规则')">
      <div v-if="steps <= 0">
            ...
            >
              ...
              >
                <template slot="limitCount" slot-scope="scope">
                  ...
                  <!--<el-input v-model="scope.row.limitCount" name="scope_row_limitCount" />-->
                </template>
              </ody-table>
            </div>
          </el-form-item>
  ...
  methods: {
    ...
    async query() {
      if (this.promotionId !== '' && this.promotionId !== undefined) {
        ...
        if (res && res.code === '0' && res.data) {
          ...
          // this.promotionActivityDetail.totalAmountLimitChecked = this.promotionActivityDetail.totalAmountLimit > 0
        }
      }
    },
    constructBackTree() {
    ...
    handleComplete(formName) {
      if (this.$refs.promotionActivityDetail) {
        ...
        //   }
        // })
        this.$refs.promotionActivityDetail.validate((valid, object) => {
          if (valid && canSave) {
            const {
              date4Customize,
