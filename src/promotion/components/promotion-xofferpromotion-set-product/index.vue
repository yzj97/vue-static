          ...
          <el-tab-pane :label="`${$t('互斥商品')}(${mutexTableListTotal})`" name="2">
            <div>
              <ody-table
                ...
                @getTableList="hanndleMutexTableList"
              >
...
export default {
  ...
  methods: {
    ...
    async querySelectionProductList(type) {
      if (type === 'effect') {
        return this.$refs.mpTable.getList(this.mpSearch)
      } else {
        return this.$refs.mutexTable.getList(this.mutexSearch)
      }
    },
    handleShowSearch() {
