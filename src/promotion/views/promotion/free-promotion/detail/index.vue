        ...
        <el-row>
          <el-col :span="10">
            <div class="detail-top-content">
              ...
              >{{ $t("当前状态") }}：{{
                detailData.activityStatus | keyVal(getActivityStatus)
              }}</span
              >
            </div>
          </el-col>
        ...
        >
          <ody-space :space="4">
            ...
            <div class="base-info">
              ...
              <div class="item">
                <label>{{ $t("活动类型") }}：</label>{{ $t("包邮促销") }}
              </div>
              <div v-show="detailData.freeShipping === 1" class="item">
                <label>{{ $t("包邮方式") }}：</label>{{ $t("单品包邮") }}
              </div>
              <div v-show="detailData.freeShipping === 2" class="item">
                <label>{{ $t("包邮方式") }}：</label>{{ $t("整单包邮") }}
              </div>
              <div class="item">
            ...
            <div class="base-info">
              ...
              <div v-show="detailData.applicablePlatformList" class="item">
                <label>{{ $t("活动平台") }}：</label>
                {{
                  detailData.applicablePlatformList
                    | getApplicablePlatform(applicableMap)
                }}
              </div>
              <div
              ...
              <div
                v-show="detailData.memberTypes && platformId != 2"
                class="item"
              >
                <label>{{ $t("会员类型") }}：</label>
                {{ memberTypeList.join("、") }}
              ...
              <div
                v-show="detailData.memberLevels && platformId != 2"
                class="item"
              >
                <label>{{ $t("会员等级") }}：</label>
                <div
                ...
                >
                  <el-tag type="info" class="levels-tag">{{
                    item.memberTypeName
                  }}</el-tag>
              ...
              <div class="item">
                <label>{{ $t("备注") }}：</label>
                {{ detailData.description | getdescription(val) }}
              </div>
            </div>
          </ody-space>
        </el-tab-pane>
            ...
            <div slot="content">
              ...
              >
                <ody-search-item :label="$t('店铺编码')" prop="storeCode">
                  <el-input
                    v-model="storeSearch.storeCode"
                    name="storeSearch_storeCode"
                  />
                </ody-search-item>
                <ody-search-item :label="$t('店铺名称')" prop="storeName">
                  <el-input
                    v-model="storeSearch.storeName"
                    name="storeSearch_storeName"
                  />
                </ody-search-item>
              </el-form>
            </div>
        ...
        <el-tab-pane :label="$t('参与商品')" name="third">
            ...
            <div slot="content">
              ...
              >
                ...
                <ody-search-item :label="$t('商品编码')" prop="obj.mpCode">
                  <el-input
                    v-model="mpSearch.obj.mpCode"
                    name="mpSearch_obj_mpCode"
                  />
                </ody-search-item>
                <ody-search-item :label="$t('商品名称')" prop="obj.mpName">
                  <el-input
                    v-model="mpSearch.obj.mpName"
                    name="mpSearch_obj_mpName"
                  />
                </ody-search-item>
                <ody-search-item :label="$t('商品条码')" prop="obj.barcode">
                  <el-input
                    v-model="mpSearch.obj.barcode"
                    name="mpSearch_obj_barcode"
                  />
                </ody-search-item>
              </el-form>
            </div>
          ...
          <ody-list-table-area>
            <div slot="tabs">
              ...
              >
                <el-tab-pane
                  :label="`${$t('生效商品')}(${mpTable.page.total})`"
                  name="1"
                >
                  <div class="bottom10">
                    <ody-button
                ...
                <el-tab-pane
                  :label="`${$t('互斥商品')}(${mutexTable.page.total})`"
                  name="2"
                >
                  <div>
                    <ody-table
                    ...
                    >
                      <template
                        ...
                        slot-scope="scope"
                      >
                        <span v-if="!!scope.row.exclusiveRecordingTime">{{
                          scope.row.exclusiveRecordingTime | parseTime
                        }}</span>
                      </template>
                    </ody-table>
                    <ody-pagination
  ...
  methods: {
    ...
    outProduct() {
      ...
      this.$confirm('确定要撤出商品吗？')
        .then((_) => {
          var arrParam = []
          for (var i = 0; i < this.checkedProductList.length; i++) {
            var item = this.checkedProductList[i]
          ...
          this.promotionAPI.deleteMpBatch(arrParam).then((res) => {
            this.$message({
    ...
    async queryPromotionActivityDetail() {
      ...
      if (code === '0' && data) {
        ...
        console.log(this.detailData, 789789)
      }
    },
    // 读取商家渠道信息
