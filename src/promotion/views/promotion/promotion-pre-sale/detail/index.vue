      ...
      <el-tabs v-model="activeName" name="activeName" @tab-click="handleTabClick">
        <el-tab-pane :label="$t('促销规则')" name="first" class="promotion-rules">
          <ody-space :space="4">
            <div class="base-info">
              ...
              <div class="item">
                <label>{{ $t('定金支付时间') }}：</label>
                {{ detailData.fromDate }} {{ $t('至') }} {{ detailData.endDate }}
              </div>
            </div>
            <div class="base-info">
              <div class="header">{{ $t('活动规则') }}</div>
              <div class="item">
                <label>{{ $t('预售类型') }}：</label>{{ $t('定金（分定金、尾款两阶段支付）') }}
              </div>
              <div class="item">
                <label>{{ $t('预售价模式') }}：</label>{{ $t('一口价') }}
              </div>
              <div class="item">
                <label>{{ $t('尾款支付时间') }}：</label>
                {{ detailData.preSellStartTime }} {{ $t('至') }}{{ detailData.preSellEndTime }}
              </div>
              <div class="item">
                <label>{{ $t('开始发货时间') }}：</label>
                {{ detailData.deliveryTimeString }}
              </div>
              <div class="item">
                <label>{{ $t('定金是否可退') }}：</label>
                <label v-if="detailData.canReturnPremoney===0">{{ $t('否') }}</label>
                <label v-else>{{ $t('是') }}</label>
              </div>
              <div class="item">
            ...
            <div v-show="platformId != 2" class="base-info">
              ...
              <div class="item">
                <label>{{ $t('是否包邮') }}：</label>
                <label v-if="detailData.freeShipping===2">{{ $t('是') }}</label>
                <label v-else>{{ $t('否') }}</label>
              </div>
            </div>
            <div class="base-info">
              ...
              <div class="item remarks">
                <label>{{ $t('备注') }}：</label>
                <label v-html="detailData.description" />
              </div>
            </div>
          </ody-space>
        </el-tab-pane>
        ...
        <el-tab-pane :label="$t('参与商家/店铺')" name="second">
          ...
          <ody-pagination
            ...
            @size-change="handleSizeChangeMerchant(1)"
            @current-change="handleMerchantSubmit(1)"
          />
          ...
          <ody-pagination
            ...
            background
            layout="total, sizes, prev, pager, next, jumper"
            @size-change="handleSizeChangeMerchant(2)"
            @current-change="handleMerchantSubmit(2)"
          />
        </el-tab-pane>
        ...
        <el-tab-pane :label="$t('参与商品')" name="third">
          <ody-list-search-area>
            ...
            <div slot="btn">
              ...
              <el-button size="small" type="primary" @click.prevent="queryData()">{{ $t('查询') }}</el-button>
            </div>
          </ody-list-search-area>
                ...
                <el-tab-pane :label="`${$t('互斥商品')}(${mutexTable.page.total})`" name="2">
                  <div>
                    ...
                    <ody-pagination
                      ...
                      background
                      layout="total, sizes, prev, pager, next, jumper"
                      @size-change="handleSizeChange('mutex')"
                      @current-change="handleCurrentChange('mutex')"
                    />
                  </div>
                </el-tab-pane>
              </el-tabs>
...
export default {
    ...
    return {
      ...
      mpTable: {
        columns: [
          ...
          {
            label: this.$t('预售价'),
            prop: 'presellTotalPrice',
            minWidth: 120, show: true
          },
          {
            label: this.$t('定金'),
            prop: 'presellDownPrice',
            minWidth: 120, show: true
          },
          {
            label: this.$t('定金抵扣金额'),
            prop: 'presellOffsetPrice',
            minWidth: 120, show: true
          },
          {
  ...
  computed: {
    ...
    getActivityTypeList() {
      return packagesConstant.promotionActivityTypeList.reduce((rtv, item) => {
        rtv[item.value] = item.label
        return rtv
      }, {})
    },
    getPromotionMethodList() {
      return constants.singlePromotionContentType.reduce((rtv, item) => {
  ...
  methods: {
    queryData() {
      const params = +this.selectFlag === 1 ? 'effect' : 'mutex'
      this.query(params)
    },
    ...
    async loadChildren(tree, treeNode, resolve) {
      ...
      res.data && res.data.forEach(item => {
        ...
        if (!util.isEmpty(item.agentList[0])) {
          item.bookNum = item.agentList[0].bookNum
          item.presellDownPrice = item.agentList[0].presellDownPrice
          item.presellOffsetPrice = item.agentList[0].presellOffsetPrice
          item.presellTotalPrice = item.agentList[0].presellTotalPrice
    ...
    async handleSizeChangeMerchant(merchantType) {
      if (merchantType === 1) {
    ...
    async handleSizeChange(merchantType) {
      ...
      await this.querySelectionProductList(merchantType)
    },
    ...
    async querySelectionProductList(type) {
      ...
      if (type === 'effect') {
        ...
        const newParams = JSON.parse(JSON.stringify(this.mpSearch))
        newParams.obj.isMutexList = [-1]
        params = newParams
      }
      ...
      selectedProductList.forEach(x => {
        ...
        merchantProductList &&
          merchantProductList.forEach(y => {
            ...
            x.bookNum = y.agentList[0].bookNum
            x.presellDownPrice = y.agentList[0].presellDownPrice
            x.presellOffsetPrice = y.agentList[0].presellOffsetPrice
            x.presellTotalPrice = y.agentList[0].presellTotalPrice
            y.bookNum = y.agentList[0].bookNum
            y.presellDownPrice = y.agentList[0].presellDownPrice
            y.presellOffsetPrice = y.agentList[0].presellOffsetPrice
            y.presellTotalPrice = y.agentList[0].presellTotalPrice
            x.salePrice = y.salePrice
