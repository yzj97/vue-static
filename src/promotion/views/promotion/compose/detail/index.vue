        ...
        <el-tab-pane :label="$t('促销规则')" name="first" class="promotion-rules">
          <ody-space :space="4">
            ...
            <div class="base-info">
              ...
              <div class="item">
                <label>{{ $t('活动类型') }}</label>
                {{ detailData.promotionType | keyVal(getPromotionTypeList) }}
              </div>
              <!-- <div class="item">
                <label>活动形式：</label>
                {{ detailData.activityType | keyVal(getActivityTypeList) }}
              </div> -->
              <div class="item">
...
export default {
  ...
  computed: {
    ...
    getConditionType() {
      return constants.repurchasePromotionContentType.reduce((rtv, item) => {
        rtv[item.id] = item.text
        return rtv
      }, {})
    },
    getMethodName() {
      if (this.detailData.promotionMethod === 12) {
        return `付${this.detailData.favourableList[0].favourableContent /
          100}元任选${this.detailData.favourableList[0].favourableCondition}件`
      } else if (this.detailData.promotionMethod === 13) {
        return `任选${this.detailData.favourableList[0].favourableCondition}件付${this.detailData.favourableList[0].favourableContent}件`
      }
    },
    getActivityTypeList() {
  ...
  methods: {
    ...
    async queryPromotionActivityDetail() {
      ...
      if (code === '0' && data) {
        ...
        console.log(this.detailData, 333)
