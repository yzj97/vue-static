          ...
          <ody-space :space="4">
            ...
            <div class="base-info">
              ...
              <div class="item">
                ...
                <span v-if="detailData.isSuperposition === 1">{{ $t('是（倍量：每满一级优惠一次）') }}
              ...
              <div class="item">
                ...
                <!-- <span v-show="detailData.promotionMethod === 2 && detailData.conditionType === 2">
                  {{ $t('满') }}{{ detailData.favourableList[0].favourableCondition }}{{ $t('件') }}，{{ $t('打') }}{{ detailData.favourableList[0].favourableContent / 10 }}{{ $t('折') }}
                </span> -->
              </div>
              <div class="item">
                {{ $t('备注') }}：
  ...
  methods: {
    ...
    async loadChildren(tree, treeNode, resolve) {
      ...
      res.data && res.data.forEach(item => {
        item.disable = true
        if (item.agentList && item.agentList[0] && !util.isEmpty(item.agentList[0].contentValue)) {
