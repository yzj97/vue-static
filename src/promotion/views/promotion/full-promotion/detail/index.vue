        ...
        <el-tab-pane :label="$t('促销规则')" name="first" class="promotion-rules">
          <ody-space :space="4">
            ...
            <div class="base-info">
              ...
              <div
                class="item"
              >{{ $t('选品范围') }}：{{ detailData.selectionRange | keyVal(getSelectionRangeTypeList) }}</div>
              <div
                class="item"
              >{{ $t('活动类型') }}：{{ detailData.promotionMethod | keyVal(getPromotionMethodList) }}</div>
              <div class="item">
            ...
            <div class="base-info">
              ...
              <div class="item">{{ $t('是否排斥优惠券') }}：{{ detailData.canUseCoupon | keyVal(getCanUseCouponList) }}</div>
              <!-- <div
                v-if="platformId === 1 "
                class="item"
              >
            ...
            <div class="base-info">
              ...
              <div class="item">
                <span
                  v-if="detailData.isSuperposition === 0"
                >{{ $t('否（超量：按最高层级享受优惠）') }}</span>
                <span v-if="detailData.isSuperposition === 1">
                  {{ $t('是（倍量：每满一级优惠一次）') }}
                  ，上限{{ detailData.giftLimit4Multy }}次
                </span>
              </div>
              <div class="item">
                {{ $t('优惠设置') }}：
...
export default {
  ...
  async mounted() {
    ...
    console.log(this.platformId, 555)
  ...
  methods: {
    ...
    async loadChildren(tree, treeNode, resolve) {
      ...
      res.data && res.data.forEach(item => {
        item.disable = true
        if (!!item.agentList && !util.isEmpty(item.agentList[0].contentValue)) {
