        ...
        <el-tab-pane :label="$t('促销规则')" name="first" class="promotion-rules">
          <ody-space :space="4">
            ...
            <div class="base-info">
              ...
              <div class="item">
                <label>{{ $t('选品范围') }}：</label>
                {{ detailData.selectionRange | keyVal(getActivityTypeList) }}
              </div>
              <!-- {{ detailData.promotionType }} -->
              <div class="item">
                <label>{{ $t('活动类型') }}：</label>
                {{ frontPormotionTypeObj[detailData.frontPormotionType] }}
              </div>
              <div class="item">
...
export default {
  ...
  computed: {
    ...
    getActivityTypeName() {
      if (this.detailData.isSuperposition === 0) {
        return '否（按最高层级仅减一次）'
      } else if (this.detailData.isSuperposition === 1) {
        return `是（每满一级优惠一次），上限${this.detailData.giftLimit4Multy}次`
      }
    },
    getActivityTypeList() {
      return packagesConstant.selectionRangeTypeList.reduce((rtv, item) => {
        rtv[item.value] = item.label
        return rtv
      }, {})
    },
    getPromotionMethodList() {
      return constants.xofferPromotion.reduce((rtv, item) => {
