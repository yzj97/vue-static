        ...
        <ody-list-search-area>
          <div slot="content">
            <el-form ref="form" class="form">
              ...
              <ody-search-item :label="$t('distributor_product_category')" prop="categoryId">
          ...
          <div slot="table">
            <el-table
              ...
              <el-table-column
                ...
                <template slot-scope="scope">
                  <div
                    v-if="scope.row.typeOfProduct === 3 || scope.row.typeOfProduct === 0">
...
export default {
  ...
  methods: {
    ...
    async loadSerialProduct(row) {
      ...
      // 先获取虚品对应的店铺商品id
      const res = await this.$pms.$api.common.getSerialProductByParam(params)
      if (res.code === '0' && res.data) {
        ...
        const result = await this.$pms.$api.common.merchantProductList(serialParams)
        if (result.code === '0') {
      ...
      if (this.serialProductList && this.serialProductList.length > 0) {
        this.serialProductList.forEach(item => {
          vue.$set(item, 'productItemNum', 1)
          vue.$set(item, 'productSpuCode', row.code)
        })
      ...
      for (let index = 0; index < this.serialProductList.length; index++) {
        ...
        for (const p in addProduct) {
          if (serialProduct[p]) {
            ...
            addProduct.productSpuCode = this.selectedProduct.code
          }
        }
        // 添加原商品没有的属性
...
async function loadList(vue, codes) {
  if (vue.searchForm.brandName) {
    ...
    const result = await vue.$pms.$api.common.merchantBrandList(brandParams)
...
async function queryProductInner(vue, codes) {
  ...
  param.warehouseType = 0 // 查询有实体仓的
  const res = await vue.$pms.$api.common.merchantProductList(param)
  if (res.code === '0') {
    if (res.data.total && res.data.total !== 0) {
      await queryContractByParam(vue, res.data.listObj)
    }
    vue.list = res.data.listObj
    vue.list.forEach(data => {
      vue.$set(data, 'productItemNum', 1)
    })
...
async function queryContractByParam(vue, productList) {
  ...
  const result = await vue.$pms.$api.thirdData.getContract({
