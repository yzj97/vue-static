<template>
  <div class="cp-task-integral-modal">
    <ody-dialog
    ...
    >
          ...
          >
            ...
            <el-form-item>
              <div class="inner-form">
                <el-form-item :label="$t('积分数值')" class="inner-form-item" prop="integral">
                  <ody-input-number v-model="innerNodeInfo.integral" :min="1" :max="999999" name="innerNodeInfo_integral">
                    <template slot="append">{{ $t('积分') }}</template>
                  </ody-input-number>
                </el-form-item>
      ...
      <span slot="footer">
        ...
        <ody-button
          name="CrmTaskListSavePoint_handleSubmit"
          size="small"
          type="primary"
          code="CrmTaskListSavePoint"
          @click="handleSubmit"
        >{{ $t('保存') }}</ody-button>
      </span>
    </ody-dialog>
  ...
  data() {
    return {
      ...
      defaultNodeInfo: {
        ...
        sendMethod: 1, // 积分设置 1: 发放 2:扣减
        integral: null, // 积分
        channelCode: null, // 渠道
        sendReason: null, // 积分理由
      ...
      rules: {
        ...
        integral: [
          { required: true, message: this.$t('请输入积分'), trigger: 'change' }
        ],
        channelCode: [
