      ...
      <el-tabs v-model="activeName" name="activeName" tab-position="left" @tab-click="handleClick">
        ...
        >
          <el-form ref="form" :rules="baseInfoRules" :model="userBaseInfo" label-width="180px">
            ...
            <div class="gap" />
            <ody-title :title="$t('InviterInformation')" />
            <el-form-item :label="$t('inviter')" class="inner-form">
              <el-input v-model="userBaseInfo.normalInviteUser" name="userBaseInfo_normalInviteUser" disabled />
            </el-form-item>
            <el-form-item :label="$t('TheInvitationTime')" class="inner-form">
              <el-date-picker
                v-model="userBaseInfo.inviteDate"
                name="userAccount_inviteDate"
                type="datetime"
                disabled
              />
            </el-form-item>
            ...
            <ody-list-search-area>
              <div slot="content">
                <el-form
                  ...
                  class="form"
                >
                  <ody-search-item :label="$t('coupons_name')" prop="couponActivityName">
                    <el-input
        ...
        <el-tab-pane v-if="$portal.hasPermission('30890357')" :label="$t('transaction_info')" name>
          ...
          <ody-list-search-area>
            <div slot="content">
              <el-form ref="form" :model="search" label-width="100px" class="form">
                ...
                <ody-search-item :space="3" :label="$t('orderCreateTime')" prop="data.orderCreateTime">
...
export default {
  ...
  methods: {
    ...
    async queryInvoiceQualification() {
      const invoiceQualification = this.$crm.$api.invoiceQualification
      ...
      this.$refs['invoiceForm'].validate(valid => {
        if (valid) {
          const invoiceQualification =
            self.$crm.$api.invoiceQualification
    ...
    async loadAreaList(parentCode, level) {
      if (parentCode) {
        const res = await this.$crm.$api.area.queryArea({
    ...
    tolist() {
      this.$router.push({
        name: this.authEnable === true ? 'OuserCenterMemberListStore' : 'OuserCenterMemberList',
        query: {}
      })
    },
