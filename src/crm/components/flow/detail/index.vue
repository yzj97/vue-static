    ...
    <section>
      <ody-list-search-area>
        ...
        <div slot="btn">
          ...
          <ody-button
            :code="type === 2 ? 'OuserCenterBalanceFlowListDetailQuery' : 'OuserCenterPointsFlowDetailQuery'"
            name="handleSearchSubmit"
            size="small"
            type="primary"
            @click="handleSearchSubmit">{{ $t('query')
            }}
          </ody-button>
        </div>
      </ody-list-search-area>
      <div class="row">
        ...
        >{{ $t('member_cell_phone_number') }}：{{ userObj.mobile }}
        </div>
      </div>
      <ody-list-table-area>
        <div slot="table">
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          minWidth: 120,
          formatter: (row, column, cellValue) => {
            return JSON.parse(row.changeDetail).memberLevelName
          }
        },
        {
          show: true,
  ...
  methods: {
    ...
    back() {
      ...
      var seeView = this.type === 2
        ? 'OuserCenterBalanceFlowListSee'
        : 'OuserCenterPointsFlowSee'
      this.$portal.delActiveViewAndRefresh(seeView)
    },
    init() {
...
async function loadList(vue) {
  ...
  let op
  if (vue.type === 2) {
    op = vue.$crm.$api.crmService.listGrowthLogPage(vue.searchForm)
  } else {
    op = vue.$crm.$api.crmService.listUserPointLogPage(vue.searchForm)
  }
  op.then(res => {
