    ...
    <el-container>
      <el-main >
        <el-row>
          <el-col v-if="step === 1" :span="10" :offset="7">
            <el-form ref="typeForm" :disabled="!add && !edit" :model="obj" label-width="100px" class="form" label-position="right">
              <el-form-item :label="$t('规则类别')+':'" :rules="[{ required: true, message: this.$t('请选择规则类型'), trigger: 'change' }]" prop="subType">
                <el-select v-model="obj.subType" :popper-append-to-body="false" :placeholder="$t('请选择')" :disabled="!add" name="obj_subType" @change="changeType">
                  <el-option
          ...
          <normal-invite v-if="step === 2 && obj.subType === '1_11'" ref="detailForm" :param="obj.paramJson" :disabled="!add && !edit" />
          <product-like v-if="step === 2 && obj.subType === '1_12'" ref="detailForm" :param="obj.paramJson" :disabled="!add && !edit" />
...
import ProductLike from '@/components/pointsManagement/ruleSteps/productLike'
import NormalInvite from '@/components/pointsManagement/ruleSteps/normalInvite'
...
export default {
  ...
  async mounted() {
    if (this.$route.query.add.toString() === '1') {
      ...
      await this.$crm.$api.crmService.getPointRuleById({ id: this.$route.query.id }).then(res => {
        this.obj = {
          id: res.data.id,
    ...
    await this.$crm.$api.userService.configListMulti({ categorys: ['POINT_RULE_TYPE_NEW'], pool: 'crm' }).then(resp => {
