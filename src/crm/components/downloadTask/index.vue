...
export default {
  name: 'DownloadTask',
  props: {
    exportType: { type: String, required: false, default: null },
    isDataTask: { type: Boolean, required: false, default: null },
  ...
  methods: {
    async init() {
      let req = this.isDataTask
        ? this.$crm.$api.common.dataTask_listPage
        : this.$crm.$api.common.downloadTask
      if (this.search) {
        req = this.search
      }
