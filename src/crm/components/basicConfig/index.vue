...
export default {
  ...
  methods: {
    async init() {
      try {
        this.$crm.$api.crmService.getRuleByType({ type: 1, subType: 1, entityType: this.type }).then(res => {
          if (res.data.param) {
    ...
    saveBasicConfig() {
      ...
      let op
      if (this.value.id) {
        op = this.$crm.$api.crmService.updateRule({ paramJson: this.value.param, id: this.value.id })
      } else {
        op = this.$crm.$api.crmService.addRule({
