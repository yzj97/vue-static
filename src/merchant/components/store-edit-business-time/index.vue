<template>
  <div class>
    <merchant-title>{{ $t('营业时间') }}</merchant-title>
    <el-form
      ref="businessTime"
      :model="businessTime"
      :rules="rules"
      ...
      label-width="200px"
    >
      <el-form-item :label="$t('是否接单')">
        <el-checkbox
          :true-label="1"
          :false-label="0"
          v-model="businessTime.businessState"
          name="businessTime_businessState"/>
      </el-form-item>
      <el-form-item
        v-for="(businessDay, index) in businessTime.businessDays"
        :key="index"
        :label="$t('星期' + (index + 1))"
      >
        <el-radio-group v-model="businessDay.type" name="businessDay_type">
          <el-radio :label="1">{{ $t('自定义时间') }}</el-radio>
          <el-radio :label="2">{{ $t('24小时营业') }}</el-radio>
      ...
      <el-form-item>
        <ody-button
          name="MerchantStoreListSaveBussinessTime_handleSubmit"
          type="primary"
          code="MerchantStoreListSaveBussinessTime"
          @click="handleSubmit"
        >{{ $t('保存') }}</ody-button>
      </el-form-item>
    </el-form>
...
export default {
  ...
  async mounted() {
    this.businessTime.businessDays = await this.queryStoreBusinessTime()
  },
  methods: {
    async queryStoreBusinessTime() {
      const { orgId } = this.$route.params

      const {
        data = []
      } = await this.$merchant.$api.storeService.queryStoreBusinessTimeInfo({
        orgId,
        timeType: 1
      })

      if (data && data.length) {
        this.businessTime.isInitData = 0
        this.businessTime.businessState = data[0].businessState
        // console.log('businessState', this.businessTime.businessState)
      ...
      this.businessTime.isInitData = 1
      ...
      return [
        {
          ...
          orgId: orgId,
          businessState: 1,
          timeType: 1, // 日期类型：1/营业时间  2:/配送时间
          type: 3, // 日历类型【1自定义】【2全天】【3歇业】
          week: 1,
          isInitData: true,
    ...
    getErrorWeekList() {
      const customBusinessDays = this.businessTime.businessDays.filter(
        x => x.type === 1
      )
      const rtv = []
    ...
    async handleSubmit() {
      const { orgId } = this.$route.params
      const { ...params } = this.businessTime
      ...
      await this.$merchant.$api.storeService.updateStoreBusinessTime(params)
      await this.queryStoreBusinessTime()
      this.$message({
        type: 'success',
        message: this.$t('保存成功')
      })
