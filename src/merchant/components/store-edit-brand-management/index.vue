<template>
  <div class="">
    <merchant-title>{{ $t('店铺经营品牌') }}</merchant-title>
    <el-form :disabled="isView || !loaded || all.length === 0">
      ...
      <div style="margin: 15px 25px">
        <ody-button name="MerchantStoreListBusinessBrand_handleSubmit" code="MerchantStoreListBusinessBrand" type="primary" @click="handleSubmit">{{ $t('保存') }}</ody-button>
      </div>
    </el-form>
...
export default {
  ...
  methods: {
    async handleSubmit() {
      ...
      await this.$merchant.$api.storeService.saveStoreBrand({
        orgId,
        storeId: orgId,
        brandIds: this.checked,
    ...
    async getChecked() {
      ...
      const { data = [] } = await this.$merchant.$api.storeService.listStoreBrand({
        orgId,
        storeId: orgId,
        brandIds: [],
        merchantId: this.merchantId
      ...
      return [...new Set(data.map(x => x.brandId))]
    },
    async getAll() {
      const { orgId } = this.$route.params

      const { data } = await this.$merchant.$api.storeService.getBrandData({
        orgId,
        storeId: orgId,
        brandIds: [],
        merchantId: this.merchantId,
