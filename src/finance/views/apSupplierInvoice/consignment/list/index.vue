  ...
  <section class="pg-apSupplierInvoice-list">
    <ody-list-search-area>
      <div slot="content">
        <el-form ref="form" label-width="100px" class="form">
          ...
          <ody-search-item :label="$t('settle_obj_type') + ':'" prop="settleObjType">
            <el-select v-model="searchForm.settleObjType" :placeholder="$t('common_all')" name="searchForm_settleObjType">
              <el-option v-for="(v, k) in settleObjTypeMap" :label="v" :key="k" :value="k" />
            </el-select>
          </ody-search-item>
          <ody-search-item :label="$t('settle_obj_name') + ':'" prop="settleObjName">
            <el-input v-model="searchForm.settleObjName" name="searchForm_settleObjName" maxlength="25" />
          </ody-search-item>
      ...
      <div slot="btn">
        ...
        <ody-button
          name="PurchaseInvoiceQuery_handleSearchSubmit"
          code="PurchaseInvoiceQuery"
          size="small"
          type="primary"
    ...
    <ody-list-table-area>
      <div slot="btn-bottom">
        <ody-button
          name="PurchaseInvoiceAdd_showChkSupplierPurchaseModal"
          code="PurchaseInvoiceAdd"
          size="small"
          type="primary"
          @click="showChkSupplierPurchaseModal"
        >{{ $t('common_add') }}</ody-button>
        <ody-button name="PurchaseInvoiceExport_exportItem" code="PurchaseInvoiceExport" size="small" @click="exportItem">{{ $t('common_export') }}</ody-button>
      </div>
      <div slot="tabs">
      ...
      <div slot="table">
        <ody-table :loading="loading" :data="list" :columns="columns" :operates="operates" name="list897" />
      </div>
      <div slot="page">
        <ody-pagination
    ...
    <ChkSupplierSoBillModal
      :visible.sync="chkSupplierPurchaseModalVisible"
      chk-order-type="1"
      chk-status="2"
      @ok="selectChkSupplierPurchaseModal"
    />
  </section>
</template>
<script>
...
/* import ChkSupplierPurchaseModal from '@/components/apSupplierInvoice/chkSupplierPurchaseModal'*/
import ChkSupplierSoBillModal from '@/components/apSupplierInvoice/chkSupplierSoBillModal'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      chkSupplierPurchaseModalVisible: false,
      ...
      baseColumns: [
        {
          ...
          render: (h, params) => {
            ...
            if (status === 1 && invoiceStyle === 0 && showEdit) {
              ...
              deleteComponent = (
                <ody-button code='PurchaseInvoiceDelete'
                  size='mini'
                  on-click={() => {
              ...
              auditComponent = (
                <ody-button code='PurchaseInvoiceAudit'
                  size='mini'
                  on-click={() => {
            ...
            if (
              ...
              redComponent = (
                <ody-button code='PurchaseInvoiceCheck'
                  size='mini'
                  on-click={() => {
            ...
            if (
              ...
              downComponent = (
                <ody-button code='PurchaseInvoiceDownload'
                  size='mini'
                  on-click={() => {
        ...
        {
          show: true,
          prop: 'settleObjTypeText',
          label: this.$t('settle_obj_type'),
          align: 'center',
          minWidth: 124
        },
        {
  ...
  methods: {
    ...
    showChkSupplierPurchaseModal() {
      // 显示对账单
      this.chkSupplierPurchaseModalVisible = true
    },
    selectChkSupplierPurchaseModal(chkSupplierPurchases) {
      if (chkSupplierPurchases && chkSupplierPurchases.length > 0) {
        this.checkAndOpenBlue(chkSupplierPurchases)
      }
    },
    forwardToDetailPage(selectedItem, isOpen, isView, chkSupplierPurchases) {
      this.$router.push({
        name: 'FinanceInvoiceSoBillDetail',
        query: {
          ...
          isOpen: isOpen,
          isView: isView,
          chkSupplierPurchases: chkSupplierPurchases
        }
      })
    },
    ...
    _initCodes() {
      ...
      this.$finance.$api.common
        .codeSelectMulti({ categories: ['INVOICE_STYLE', 'SETTLE_OBJ_TYPE'] })
        .then(data => {
          if (data.code === '0') {
            var map = data.data
    ...
    async checkAndOpenBlue(chkSupplierPurchases) {
      const form = getDefaultSearchForm()
      form.supplierInvoiceDetail = []
      const promise = await this.$finance.$api.chkSupplierPurchase
        .newQueryChkSupplierPurchaseDetailListWithoutPage
      for (let i = 0; i < chkSupplierPurchases.length; i++) {
        const result = await promise({
          chkBillCode: chkSupplierPurchases[i].billCode
        })
        if (result.code === '0') {
          ...
          if (data && data.length > 0) {
            for (let index = 0; index < data.length; index++) {
              ...
              const row = {
                chkOrderCode: chkSupplierPurchases[i].billCode,
                refOrderCode: chkSupplierPurchases[i].refOrderCode,
      ...
      const maxItemResult = await this.$finance.$api.apSupplierInvoice.newLoadMaxItemNum(
        form
      )
    ...
    exportItem() {
      ...
      this.$portal.downloadFileByPost(
        '/api/back-finance-web/newApSupplierInvoice/newExport.do',
        filters
      )
    }
  }
