          ...
          <el-tabs v-model="activeState" name="activeState" @tab-click="handleTabsClick" >
            <el-tab-pane id="baseInfo" :label="$t('invoice_baseInfo')" >
              <el-row>
                ...
                <el-col :span="8">
                  <el-form-item :label="$t('invoice_date')" class="register" prop="invoiceTime">
                    <el-date-picker
                      v-if="!isView"
...
export default {
  ...
  methods: {
    ...
    setDefaultInfo() {
      ...
      this.dataForm.settleObjCode = selectChkSupplierPurchase.supplierCode
      this.dataForm.settleObjName = selectChkSupplierPurchase.supplierName
    },
    ...
    handleSubmit() {
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          const promise = vue.$finance.$api.apSupplierInvoice.saveInvoiceNew
    ...
    async loadChkSupplierPurchaseItem() {
      ...
      const promise = await this.$finance.$api.chkSupplierPurchase.newQueryChkSupplierPurchaseDetailListWithoutPage
      for (let i = 0; i < this.chkSupplierPurchases.length; i++) {
        const result = await promise({
          chkBillCode: this.chkSupplierPurchases[i].billCode
        })
        if (result.code === '0') {
          ...
          if (data && data.length > 0) {
            for (let index = 0; index < data.length; index++) {
              ...
              const row = {
                chkOrderCode: this.chkSupplierPurchases[i].billCode,
    ...
    async LoadMaxItemNum() { // 蓝票需要查询 采购对账单
      ...
      const result = await this.$finance.$api.apSupplierInvoice.newLoadMaxItemNum(form)
      const containList = []
      if (result.code === '0') {
