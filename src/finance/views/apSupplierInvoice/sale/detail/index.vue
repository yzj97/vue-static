<template>
  <section class="pg-apSupplierInvoice-list">
    <el-form ref="form" :rules="rules" :model="dataForm" label-width="100px" class="form">
      <ody-list-table-area>
          ...
          <el-tabs v-model="activeState" name="activeState" @tab-click="handleTabsClick" >
            <el-tab-pane id="baseInfo" :label="$t('invoice_baseInfo')">
              ...
              <el-row>
                ...
                <el-col :span="6">
                  <el-form-item :label="$t('invoice_customer_name')" required>
        ...
        <div slot="table">
          <el-table
            ...
            name="dataForm_supplierInvoiceDetail385"
            style="width: 100%">
            <el-table-column
              ...
              <template slot-scope="scope">
                <ody-button
                  name="SaleInvoiceDetailDelete_deleteRow"
                  code="SaleInvoiceDetailDelete"
                  size="mini"
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <el-form-item
                  ...
                  label-width="0px"
                >
                  <ody-input-number v-model="scope.row.itemNum" :precision="4" :step="0.1" :min="scope.row.minItemNum" :max="scope.row.maxItemNum" :controls="false" name="scope_row_itemNum" style="width: 100px;" @change="modifyItemNum(scope.row)" />
                </el-form-item>
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <el-form-item
                  ...
                  label-width="0px"
                >
                  <el-input-number v-model="scope.row.itemTaxedUnitAmt" :precision="6" :step="0.1" :min="0.000001" :max="999999999999" :controls="false" name="scope_row_itemTaxedUnitAmt" style="width: 100px;" @change="modifyItemTaxedUnitAmt(scope.row)" />
                </el-form-item>
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <el-form-item
                  ...
                  label-width="0px"
                >
                  <el-select v-model="scope.row.itemTaxRate" name="scope_row_itemTaxRate" @change="modifyItemTaxRate(scope.row)">
                    <el-option v-for="(v, k) in saleTaxRateMap" :label="v" :key="k" :value="k" />
                  </el-select>
                </el-form-item>
                <span v-if="isView || isRed" >{{ saleTaxRateMap[scope.row.itemTaxRate] ? saleTaxRateMap[scope.row.itemTaxRate] : '0.00%' }}</span>
              </template>
            </el-table-column>
            <el-table-column
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                ...
                <el-form-item
                  ...
                  label-width="0px"
                >
                  <el-input-number v-model="scope.row.itemTaxedAmt" :precision="2" :step="0.1" :min="0.01" :max="999999999999" :controls="false" name="scope_row_itemTaxedAmt" style="width: 100px;" @change="modifyItemTaxedAmt(scope.row)" />
                </el-form-item>
      ...
      <el-form-item>
        <ody-button v-if="(!isView && dataForm.status == 1) && ((merchantInfo.merchantId == dataForm.merchantId && platform === 1) || platform !== 1)" name="SaleInvoiceSave_handleSubmit" code="SaleInvoiceSave" size="small" type="primary" @click="handleSubmit">{{ $t('common_save') }}</ody-button>
        <ody-button v-if="(!isOpen && !isView && dataForm.status == 1) && ((merchantInfo.merchantId == dataForm.merchantId && platform === 1) || platform !== 1)" name="SaleInvoiceAudit_showAuditModal" code="SaleInvoiceAudit" size="small" type="primary" @click="showAuditModal">{{ $t('common_audit') }}</ody-button>
...
export default {
  ...
  methods: {
    ...
    setDefaultInfo() {
      const selectChkCustomer = this.chkCustomerSoes[0]
      this.merchantInfo.merchantName = selectChkCustomer.merchantName
      this.merchantInfo.merchantId = selectChkCustomer.merchantId
      this.dataForm.merchantId = selectChkCustomer.merchantId
      this.dataForm.merchantCode = selectChkCustomer.merchantCode
      this.dataForm.merchantName = selectChkCustomer.merchantName
      this.dataForm.storeId = selectChkCustomer.storeId
      this.dataForm.storeCode = selectChkCustomer.storeCode
      this.dataForm.storeName = selectChkCustomer.storeName

      this.dataForm.customerId = selectChkCustomer.customerId
      this.dataForm.customerCode = selectChkCustomer.customerCode
      this.dataForm.customerType = selectChkCustomer.customerType
      this.dataForm.customerName = selectChkCustomer.customerName
    },
    ...
    async loadSaleConfig() { // 获取销售方发票信息
      ...
      let config = {}
      let result = await this.$finance.$api.invoiceMerchantConfig.queryMerchantConfigWithPage({
        currentPage: 1,
        itemsPerPage: 1,
        obj: params
      })
      if (result.code === '0' && result.data && result.data.listObj && result.data.listObj.length > 0) {
        ...
        delete params.storeIsNull
        result = await this.$finance.$api.invoiceMerchantConfig.queryMerchantConfigWithPage({
          currentPage: 1,
          itemsPerPage: 1,
          obj: params
        })
    ...
    async loadPurchanseConifg() { // 获取购买方发票资质
      // 客户类型 1经销商 2加盟商 3分销商 4:商家
      // 开票类型： 开票者类型：   1 会员 2 商家 4 经销商  5 分销商 6 加盟商
      let drawerType = 1
      const customerType = this.dataForm.customerType + ''
      if (customerType === '1') {
        drawerType = 4
      ...
      const params = {
        invoiceDrawerCode: this.dataForm.customerCode,
    ...
    _initCodes() {
      ...
      this.$finance.$api.common.getConfigByKey({ configKey: 'saleTaxRateConfig' }).then(result => {
        if (result.code === '0') {
          vue.saleTaxRateMap = JSON.parse(result.data)
        }
      })
    },
    statisticInfo() {
    ...
    handleSubmit() {
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          let promise = vue.$finance.$api.apSupplierInvoice.saveInvoice
    ...
    async loadChkCustomerItem() {
      const vue = this
      const promise = await this.$finance.$api.chkCustomerSo.queryChkCustomerSoItemListWithoutPage
      for (let i = 0; i < this.chkCustomerSoes.length; i++) {
        const result = await promise({
          chkCustomerSoId: this.chkCustomerSoes[i].id
        })
        if (result.code === '0') {
          ...
          if (data && data.length > 0) {
            for (let index = 0; index < data.length; index++) {
              ...
              const row = {
                chkOrderCode: this.chkCustomerSoes[i].chkOrderCode,
                refOrderCode: this.chkCustomerSoes[i].refOrderCode,
                ...
                itemTaxRate: Number(chkItem.saleTaxRate).toFixed(2) + '',
                itemUnTaxedUnitAmt: chkItem.saleWithoutTaxPrice,
                itemTaxedUnitAmt: chkItem.saleWithTaxPrice,
    ...
    async init(update) {
      ...
      this.chkCustomerSoes = this.$route.query.chkCustomerSoes
      ...
      if (this.chkCustomerSoes && this.isOpen) { // 开蓝票才需要获取资质信息
        await this.loadChkCustomerItem()
        this.setDefaultInfo()
