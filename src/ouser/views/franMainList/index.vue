<template>
  <div>
    <section>
      <ody-list-search-area>
        <div slot="content">
          <el-form ref="form" label-width="100px" class="form">
            ...
            <ody-search-item :label="$t('所属经销商')" prop="entityId">
      ...
      <ody-list-table-area>
        <div slot="btn">
          ...
          <el-button name="batchUpdateEmployeeStatus1" size="small" @click="batchUpdateEmployeeStatus(1)">{{ $t('批量启用') }}</el-button>
        </div>
        <div slot="table">
          <ody-table
            :loading="loading"
    ...
    <employee-edit
      ...
      <el-form-item :label="$t('所属加盟商')" prop="entityId">
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        entityId: [{ required: true, message: '请选择加盟商', trigger: 'change' }]
      },
      // 列表框对象
      listTable: {
        dataKey: 'userId',
      ...
      merchantInfo: {},
      disList: [],
      identityType: { identitySubType: 61, identityType: 6 }
    }
  },
  async mounted() {
    this.loading = true
  ...
  methods: {
    ...
    async init() {
      ...
      try {
        ...
        await merchantService.getDistributorByMerchant({ merchantId: this.merchantInfo.merchantId, orgType: 6 }).then(res => {
    ...
    editDialogCommit() {
      this.$refs['editEmployee'].validate((valid) => {
        if (valid) {
          ...
          if (this.editObj.add) {
            this.editObj.obj.identitySubType = 61
            this.editObj.obj.identityType = 6
    ...
    updateEmployeeStatus(status, userIds) {
      const employeeService = this.$ouser.$api.employeeService
      const self = this
      employeeService.updateEmployeeStatus({ identitySubTypes: [61], isAvailable: status, userIds: userIds }).then(() => {
        self.$message({
          message: '信息更新成功',
          type: 'success'
        })
