    ...
    <section>
      ...
      <ody-list-table-area>
        <div slot="btn">
          ...
          <el-button name="batchUpdateEmployeeStatus5" size="small" @click="batchUpdateEmployeeStatus(1)">{{ $t('批量启用') }}</el-button>
        </div>
        <div slot="table">
          <ody-table
            :loading="loading"
...
export default {
  ...
  data() {
    return {
        ...
        operates: {
          ...
          'list': [
            ...
            },
            {
              ...
              'disabled': false,
              method: (index, row) => {
                window.document.location.href = '#/ouser-web/operaAddRole/' + row.userId + '/20'
              }
            }
          ]
        }
      ...
      // 商家列表
      merchantList: [],
      identityType: { identitySubType: 20, identityType: 2 }
    }
  },
  async mounted() {
    this.loading = true
  ...
  methods: {
    ...
    async init() {
      ...
      try {
        // 查询商家列表
        await merchantService.getAuthOrg({ type: '1' }).then(res => {
          this.merchantList = res.data
        })
        // 查询部门列表
        await userService.getCurrentOrg({}).then((res) => {
          this.org.list = res.data
          this.org.tree = util.arr2tree(res.data, 'id', 'pId', 'name')
        })
        // 查询岗位列表
    ...
    async queryEmployeePageByIdentityType() {
      ...
      this.listTable.list.forEach(one => {
        ...
        // 转换岗位id和岗位name
        one.positionName = util.getOnePropByKeyFromArr(this.positions, one.positionId, 'id', 'positionName')
        // 转换部门orgId和部门name
        one.orgName = util.getOnePropByKeyFromArr(this.org.list, one.departmentId, 'orgId', 'name')
        // 转换状态枚举和状态name
        one.isAvailableStr = util.getOnePropByKeyFromArr(this.isAvailable, one.isAvailable)
        // 商家名称
        one.entityStr = util.getOnePropByKeyFromArr(this.merchantList, one.entityId, 'id', 'name')
      })
    ...
    async updateEmployeeStatus(status, userIds) {
      const employeeService = this.$ouser.$api.employeeService
      await employeeService.updateEmployeeStatus({ identitySubTypes: [20], isAvailable: status, userIds: userIds })
