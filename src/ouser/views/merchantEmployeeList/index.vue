  ...
  <div>
    ...
    <employee-edit
      ...
      <el-form-item :label="$t('所属商家')" >
        <el-input v-model="merchantInfo.merchantName" :disabled="true" name="merchantInfo_merchantName"/>
      </el-form-item>
    </employee-edit>
  </div>
</template>
...
export default {
  name: 'OuserMerchantEmployeeList',
  components: {
    EmployeeEdit
  },
  data() {
    ...
    return {
        ...
        operates: {
          ...
          'list': [
            ...
            },
            {
              ...
              'disabled': false,
              method(index, row) {
                window.document.location.href = '#/ouser-web/operaAddVisit/' + row.userId + '/21'
              }
            }
          ]
        }
      ...
      // 商家列表
      merchantInfo: {},
      identityType: { identitySubType: 21, identityType: 2 }
    }
  },
  async mounted() {
    this.loading = true
  ...
  methods: {
    ...
    async queryEmployeePageByIdentityType() {
      ...
      self.listTable.list.forEach(one => {
        ...
        // 商家名称
        one.entityStr = self.merchantInfo.merchantName
      })
    ...
    initEditDialog(obj, edit) {
      ...
      if (obj.userId) {
        ...
        this.editObj.obj.entityId = this.merchantInfo.merchantId
    ...
    updateEmployeeStatus(status, userIds) {
      const employeeService = this.$ouser.$api.employeeService
      const self = this
      employeeService.updateEmployeeStatus({ identitySubTypes: [21], isAvailable: status, userIds: userIds }).then(() => {
        self.$message({
          message: '信息更新成功',
          type: 'success'
        })
