<template>
  <section>
    <ody-list-table-area>
      <div slot="btn">
        <AddField :only-main-fields="true" :main-fields="mainFields" :rel-table-data="relTableData" :field-table-data.sync="fieldTableData"/>
      ...
      <div slot="table">
        ...
        >
          ...
          <template slot="editRequired" slot-scope="scope">
            <el-form-item label-width="0">
              <el-checkbox v-model="scope.row.required" name="scope_row_required" />
            </el-form-item>
          </template>
          <template slot="editTag" slot-scope="scope">
            <el-form-item label-width="0">
              <el-input v-model="scope.row.tagAttributes" :readonly="true" name="scope_row_tagAttributes" placeholder="编辑控件属性" class="input-with-select">
                <el-select slot="prepend" v-model="scope.row.tag" :clearable="true" name="scope_row_tag" @change="onEditTagChange(scope.row)" @clear="onTagClear(scope.row)">
                  <el-option
...
export default {
  ...
  methods: {
    ...
    openAttrEditDialog(row, index) {
      const option = this.componentEditTypeOptions.find(item => item.code === row.tag)
      if (!option) {
        return
      }
      let tagAttributes = {}
    ...
    onEditTagChange(row) {
      const item = this.componentEditTypeOptions.find(item => item.code === row.tag)
      if (item) {
