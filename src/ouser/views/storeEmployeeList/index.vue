<template>
  <div>
    ...
    <employee-edit
      ...
      :validate-map="{ 'entityId': '所属店铺不能为空' }"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      merchantInfo: {},
      storeList: [],
      identityType: { identitySubType: 31, identityType: 3 }
    }
  },
  async mounted() {
    this.loading = true
  ...
  methods: {
    ...
    async init() {
      ...
      try {
        ...
        await StoreorgpageinfoService.queryPlatformAuthStorePage({ 'currentPage': 1, 'itemsPerPage': 200, 'merchantIds': [this.merchantInfo.merchantId] }).then(res => {
    ...
    updateEmployeeStatus(status, userIds) {
      const employeeService = this.$ouser.$api.employeeService
      const self = this
      employeeService.updateEmployeeStatus({ identitySubTypes: [31], isAvailable: status, userIds: userIds }).then(() => {
        self.$message({
          message: '信息更新成功',
          type: 'success'
        })
