<template>
  <div>
    ...
    <employee-edit
      ...
      <el-form-item :label="$t('所属经销商')" prop="entityId">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      merchantInfo: {},
      disList: [],
      identityType: { identitySubType: 41, identityType: 4 }
    }
  },
  async mounted() {
    this.loading = true
  ...
  methods: {
    ...
    async init() {
      ...
      try {
        ...
        await merchantService.getDistributorByMerchant({ merchantId: this.merchantInfo.merchantId, orgType: 4 }).then(res => {
    ...
    updateEmployeeStatus(status, userIds) {
      const employeeService = this.$ouser.$api.employeeService
      const self = this
      employeeService.updateEmployeeStatus({ identitySubTypes: [41], isAvailable: status, userIds: userIds }).then(() => {
        self.$message({
          message: '信息更新成功',
          type: 'success'
        })
