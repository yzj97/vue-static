<template>
  <div>
      ...
      <ody-list-search-area>
        <div slot="content">
          <el-form ref="form" label-width="100px" class="form">
            ...
            <ody-search-item :label="$t('所属分销商')" prop="entityId">
        ...
        <div slot="btn">
          ...
          <ody-button name="hanldeQuery" size="small" type="primary" @click="hanldeQuery">{{ $t('查询') }}</ody-button>
        </div>
      </ody-list-search-area>
      <ody-list-table-area>
        <div slot="btn">
          ...
          <el-button name="batchUpdateEmployeeStatus2" size="small" @click="batchUpdateEmployeeStatus(1)">{{ $t('批量启用') }}</el-button>
        </div>
        <div slot="table">
          <ody-table
            :loading="loading"
    ...
    <employee-edit
      ...
      <el-form-item :label="$t('所属分销商')" prop="entityId">
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        entityId: [{ required: true, message: '请选择分销商', trigger: 'change' }]
      },
      // 列表框对象
      listTable: {
        dataKey: 'userId',
        ...
        operates: {
          ...
          'list': [
            ...
            },
            {
              ...
              'disabled': false,
              method(index, row) {
                window.document.location.href = '#/ouser-web/operaAddRole/' + row.userId + '/41'
              }
            }
          ]
        }
      ...
      merchantInfo: {},
      disList: [],
      identityType: { identitySubType: 51, identityType: 5 }
    }
  },
  async mounted() {
    this.loading = true
  ...
  methods: {
    ...
    async init() {
      ...
      try {
        ...
        await merchantService.getDistributorByMerchant({ merchantId: this.merchantInfo.merchantId, orgType: 5 }).then(res => {
          self.disList = res.data.listObj
        })
        // 查询岗位列表
    ...
    async queryEmployeePageByIdentityType() {
      ...
      self.listTable.list.forEach(one => {
        ...
        // 店铺名称
        one.entityStr = util.getOnePropByKeyFromArr(self.disList, one.entityId, 'orgId', 'orgName')
      })
      this.loading = false
    },
    ...
    editDialogCommit() {
      this.$refs['editEmployee'].validate((valid) => {
        if (valid) {
          ...
          if (this.editObj.add) {
            this.editObj.obj.identitySubType = 51
            this.editObj.obj.identityType = 5
    ...
    updateEmployeeStatus(status, userIds) {
      const employeeService = this.$ouser.$api.employeeService
      const self = this
      employeeService.updateEmployeeStatus({ identitySubTypes: [51], isAvailable: status, userIds: userIds }).then(() => {
        self.$message({
          message: '信息更新成功',
          type: 'success'
        })
