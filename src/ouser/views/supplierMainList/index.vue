<template>
  <div>
      ...
      <ody-list-search-area>
        <div slot="content">
          <el-form ref="form" label-width="100px" class="form">
            ...
            <ody-search-item :label="$t('所属供应商')" prop="entityId">
        ...
        <div slot="btn">
          <el-button name="queryEmployeePageByIdentityType" size="small" type="primary" @click="queryEmployeePageByIdentityType">{{ $t('查询') }}</el-button>
          <el-button name="initForm" size="small" @click="initForm">{{ $t('重置') }}</el-button>
        </div>
      </ody-list-search-area>
      <ody-list-table-area>
        <div slot="btn">
          <el-button name="initEditDialog" size="mini" type="primary" @click="initEditDialog({},false)">{{ $t('添加') }}</el-button>
          <el-button name="batchUpdateEmployeeStatus" size="mini" type="primary" @click="batchUpdateEmployeeStatus(0)">{{ $t('批量停用') }}</el-button>
          <el-button name="batchUpdateEmployeeStatus2" size="mini" type="primary" @click="batchUpdateEmployeeStatus(1)">{{ $t('批量启用') }}</el-button>
        </div>
        <div slot="table">
          <ody-table
            :data="listTable.list"
    ...
    <employee-edit
      ...
      <el-form-item :label="$t('所属供应商')" prop="entityId">
        <el-select v-model="editObj.obj.entityId" :disabled="editObj.obj.userId" :placeholder="$t('请选择')" name="editObj_obj_entityId">
          <el-option
            v-for="item in supplierList"
            :key="item.id"
            :label="item.supplierName"
...
export default {
  ...
  data() {
    ...
    return {
        ...
        operates: {
          ...
          'list': [
            ...
            },
            {
              ...
              'disabled': false,
              method(index, row) {
                window.document.location.href = '#/ouser-web/operaAddRole/' + row.userId + '/71'
              }
            }
          ]
        }
      ...
      // 供应商列表
      supplierList: [],
      identityType: { identitySubType: 71, identityType: 6 }
    }
  },
  mounted() {
    this.init()
  ...
  methods: {
    async init() {
      const opmsService = this.$ouser.$api.opmsService
      ...
      try {
        ...
        await opmsService.querySupplierList({ currentPage: 1, itemsPerPage: 10000, obj: { merchantId: merchantId }}).then(res => {
          self.supplierList = res.data.listObj
        })
        // 查询岗位列表
        await userService.queryPositionList({ 'entityType': 1, 'currentPage': 1, 'itemsPerPage': 500 }).then((res) => {
    ...
    queryEmployeePageByIdentityType() {
      ...
      const self = this
      // 转换部门id和orgId
      employeeService.queryEmployeePageByIdentityType(this.searchForm).then((res) => {
        self.listTable.total = res.data.total
        self.listTable.list = res.data.listObj
        self.listTable.list.forEach(one => {
          one.positionId = one.positions[0]
          // 转换岗位id和岗位name
          ...
          // 店铺名称
          one.entityStr = util.getOnePropByKeyFromArr(self.supplierList, one.entityId, 'id', 'supplierName')
        })
      })
    },
    ...
    editDialogCommit() {
      this.$refs['editEmployee'].validate((valid) => {
        if (valid) {
          ...
          if (this.editObj.add) {
            this.editObj.obj.identitySubType = 71
            this.editObj.obj.identityType = 7
    ...
    updateEmployeeStatus(status, userIds) {
      const employeeService = this.$ouser.$api.employeeService
      const self = this
      employeeService.updateEmployeeStatus({ identitySubTypes: [71], isAvailable: status, userIds: userIds }).then(() => {
        self.$message({
          message: '信息更新成功',
          type: 'success'
        })
