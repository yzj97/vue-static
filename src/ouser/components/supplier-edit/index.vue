        ...
        >
          <el-form-item :label="$t('组织架构')" prop="departmentId">
            <ody-organization-select
              ...
              @select="changeDepartmentId"
            />
          </el-form-item>
          <el-form-item :label="$t('供应商名称')" prop="orgName">
            <el-input
              v-model="supplier.basicInfo.orgName"
          ...
          <el-form-item :label="$t('供应商简称')" prop="shortName">
            <el-input
              v-model="supplier.basicInfo.shortName"
              name="supplier_basicInfo_shortName"
              maxlength="100"
            />
          </el-form-item>
        </el-form>
...
export default {
  ...
  async mounted() {
    if (this.isEdit || this.isView) {
      ...
      const {
        data: basicInfo = {}
      } = await this.$ouser.$api.supplier.queryDistributionDetailInfoByOrgId({
      ...
      } = await this.$ouser.$api.supplier.selectMerchantAccount({
  ...
  methods: {
    async handleSupplierSubmit() {
      ...
      var saveReturnData = await this.$ouser.$api.supplier.saveOrUpdateDistribution(
        params
      )
    ...
    saveSupplierAccount(supplierAccountInfo) {
      ...
      return this.$ouser.$api.supplier.saveMerchantAccount(params)
    },
    allTabFormValidate() {
      const formValidates = []
    ...
    changeDepartmentId() {
      this.$refs['info'][0].changeClassification()
    },
    returnBack() {
