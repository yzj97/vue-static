<template>
  <ody-dialog
    ...
    <section class="pg-export-task-list">
      ...
      <ody-list-table-area>
        <div slot="table">
          <el-table :data="taskList" name="taskList495" size="small" style="width: 100%">
            <el-table-column :label="$t('taskCode')" :prop="isDataTask ? 'id' : 'asyncId'" width="80" />
            ...
            <el-table-column :label="$t('Totalnumberofrecords')" :prop="isDataTask ? 'totalCount' : 'records'" width="80" align="right"/>
            <el-table-column :label="$t('startDate')" :prop="isDataTask ? 'startTime' : 'createTime'" width="160" align="center"/>
            <el-table-column :label="$t('endDate')" :prop="isDataTask ? 'endTime' : 'createFileTime'" width="160" align="center"/>
            <el-table-column :label="$t('Operator')" prop="createUsername" width="120" align="center"/>
            <el-table-column :label="$t('Clickdownloads')" width="80" align="center">
              <template slot-scope="scope">
                <a
                  v-if="isDataTask ? scope.row.filePath : scope.row.downPath"
                  name="download"
                  style="color:#0066ff;cursor:pointer;"
                  @click="download(isDataTask ? scope.row.filePath : scope.row.downPath, scope.row.fileName)"
                >{{ $t('Clickdownloads') }}</a>
              </template>
            </el-table-column>
          </el-table>
  ...
  methods: {
    ...
    async handleSearchSubmit() {
      let req = this.isDataTask
        ...
        : this.$api.dataTask.asyncExportTask
      if (this.search) {
        req = this.search
      }
    ...
    async _loadDataTask(req) {
      let filterData = {
        pool: this.pool, type: 2, taskType: this.taskType
      }
      if (this.taskId) {
      ...
      if (this.startTimeRange && this.startTimeRange.length > 1) {
        filterData.startStartTime = this.startTimeRange[0] + ' 00:00:00'
        filterData.endStartTime = this.startTimeRange[1] + ' 23:59:59'
      }
      ...
      result.result.list.forEach(row => {
        ...
        row.createFileTime =
          row.createFileTime != null
            ? this.$portal.parseTime(row.createFileTime, '{y}-{m}-{d} {h}:{i}:{s}')
            : ''
      })
