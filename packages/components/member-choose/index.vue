<template>
  <div class="cp-member-choose">
    <template v-if="multiple || type === 'button'">
      <div class="my-button" @click="visible = true">
        <slot>
          <el-button v-bind="$attrs" type="default" size="small" > + {{ $t("选择会员") }}</el-button>
        </slot>
      </div>
    </template>
    <template v-else>
...
export default {
  ...
  data() {
    return {
      ...
      choose: {
        ...
        requestUrl: '/ouser-web/api/user/getUserOnlyByConditionsWithPage.do',
        columns: [
          {
            label: this.$t('会员号'),
            prop: 'id',
  ...
  methods: {
    async initSelected() {
      ...
      params[this.valueKey] = this.value ? this.value + '' : null
      const resp = await this.$oms.$api.common.userList(params)
      this.handleSubmit(resp.data.listObj)
    },
    handleClear() {
      this.$emit('input', '')
